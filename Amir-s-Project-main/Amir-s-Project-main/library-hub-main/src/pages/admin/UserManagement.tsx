...
interface UserRow extends UserFormData {
  id: number;
  name: string;
  email: string;
...
const UserManagement = () => {
  ...
  const handleSubmit = (data: UserFormData) => {
    if (editUser) {
      setUsers(prev => prev.map(u => u.id === data.id ? { ...data, id: data.id, name: data.name || "", email: data.email || "", role: data.role as "Admin" | "Librarian" | "Patron", status: data.status as "Active" | "Inactive", barcode: generateBarcode(data.id || 0) } : u));
    } else {
      ...
      const newUser: UserRow = {
        ...
        name: data.name || "",
        email: data.email || "",
        role: data.role as "Admin" | "Librarian" | "Patron",
        status: data.status as "Active" | "Inactive",
        barcode: generateBarcode(newId),
      };
  ...
  return (
    <DashboardLayout items={adminNav} title="LibraryMS" roleLabel="Admin">
      ...
      <DataTable
        columns={[
          { header: "Name", accessor: (row: UserRow) => row.name },
          { header: "Email", accessor: (row: UserRow) => row.email },
          { header: "Barcode", accessor: (row: UserRow) => (
